  ...
  <div class="fs-habilidades-topo__conteudo">
    ...
    <div class="fs-habilidades-titulo fs-animar">
      ...
      <p class="fs-habilidades-titulo__desc">
        Skills são as capacidades do seu Copilot. Com o modelo de ativações da FULL, cada assento dá acesso a toda a
        pilha — você ativa o que quiser, quando quiser, sem pagar por ativação
        individual.
      </p>
    </div>

    <div class="fs-habilidades-abas-wrapper fs-animar fs-animar--d1">
      <button class="fs-habilidades-abas-wrapper__seta fs-habilidades-abas-wrapper__seta--esq fs-oculto" id="abaSetaEsq"
      ...
      <ul class="fs-habilidades-abas nav" id="abasHabilidades" role="tablist">
        <?php foreach ($repo->all() as $skill): ?>
          <li class="nav-item" role="presentation">
            <button class="fs-habilidades-abas__item <?= $skill === $starterSkill ? 'active' : ''; ?>"
              id="tab-<?= $skill::ID ?>" data-bs-toggle="tab" data-bs-target="#pane-<?= $skill::ID ?>" type="button"
              role="tab">
      ...
      <button class="fs-habilidades-abas-wrapper__seta fs-habilidades-abas-wrapper__seta--dir fs-oculto" id="abaSetaDir"
...
<main class="tab-content" id="conteudoAbas">
  <?php foreach ($repo->all() as $skill): ?>
    <div class="tab-pane fade show <?= $skill === $starterSkill ? 'active' : ''; ?>" id="pane-<?= $skill::ID ?>"
      role="tabpanel" aria-labelledby="tab-<?= $skill::ID ?>">
          ...
          <?php if ($skill->isAvailable()): ?>
            <div class="fs-habilidades-grade__linha">
              ...
              <?php foreach ($skill->actions() as $action):
                if (!$action->showInActionsDropdown()) {
                  continue;
                }
                ?>
                <div class="fs-cartao-plugin <?= $action->isAvailable() ? '' : 'fs-cartao-plugin--bloqueado' ?>">
                  <div class="fs-cartao-plugin__info">
                    <div class="fs-aba-embreve__icone">
                      <img src="<?= $fs->getUrl($action->getIcon()) ?>" alt="<?= $action->getName() ?>" width="24"
                        height="24">
                    </div>
                ...
                <ul class="fs-aba-embreve__lista">
                  <?php foreach ($skill->getFeaturesList() as $feature): ?>
                    <li class="fs-aba-embreve__item">
                      <img src="<?= $fs->getUrl('assets/images/icons/checkmark-circle.svg') ?>" alt="" width="18"
                        height="18" />
                      <span><?= $feature ?></span>
                    </li>
                  <?php endforeach; ?>
